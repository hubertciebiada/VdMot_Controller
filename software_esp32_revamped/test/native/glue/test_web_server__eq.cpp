...
// login lock that ends between the guard and the login check, and a mark left at a request
// address that the heap hands out again.
#include <string>
...
TEST_CASE("web body: a mark left by an upload that went away does not answer a bodyless request") {
  ...
  // A sends two files: the first while B uploads (A is marked busy), the second after B
  ...
  AsyncWebServerRequest* const addressOfA = ea.request();
  ea.disconnect();  // before the end of its body: the upload is aborted
